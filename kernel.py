...
def evaluate_policy(args, env, agent, state_norm):
    ...
    for _ in range(times):
        ...
        while not done:
            ...
            a = agent.evaluate(s) # lấy hành động từ agent dựa trên trạng thái
...
def main(args, seed, target_rate, ROOT_PATH=None, load_path=None, s_mean_std=None):
    ...
    # Đặt seed ngẫu nhiên cho môi trường
    env.seed(seed)
    ...
    if not load_path:
        ...
        # Viết lại các thông số đào tạo hiện tại
        ...
        # Ghi lại vòng đánh giá
        evaluate_num = 0  # Record the number of evaluations (evalution_policy)
        ...
        with open(ROOT_PATH + '/episode_rewards.csv', mode='w', newline='') as file:
            ...
            writer.writerow(['Episode Number', 'Episode Reward'])
        ...
        for episode_num in range(1):
            ...
            while not done:
                ...
                # Action and the corresponding log probability
                a, a_logprob = agent.choose_action(s)
                # Nhận hành động dựa trên phân phối đầu ra
                if args.policy_dist == "Beta":
                    # [0,1]->[-max,max]
                    action = 2 * (a - 0.5) * args.max_action
                else:
